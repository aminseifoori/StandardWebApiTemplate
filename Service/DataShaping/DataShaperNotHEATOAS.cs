...
public class DataShaperNotHEATOAS<T> : IDataShaperNotHEATOAS<T> where T : class
{
    public PropertyInfo[] Properties { get; set; }

    public DataShaperNotHEATOAS()
    {
        Properties = typeof(T).GetProperties(BindingFlags.Public | BindingFlags.Instance);
    }

    public IEnumerable<Entity> ShapeDataNH(IEnumerable<T> entities, string fieldsString)
    {
        var requiredProperties = GetRequiredProperties(fieldsString);

        return FetchData(entities, requiredProperties);
    }
    public Entity ShapeDataNH(T entity, string fieldsString)
    {
        var requiredProperties = GetRequiredProperties(fieldsString);
